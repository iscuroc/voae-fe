...
const Calendario: React.FC = () => {
  ...
  const [selectedActivities, setSelectedActivities] = useState<
    ActividadEstado[]
  >([]); // Agregado para almacenar las actividades del día seleccionado
  ...
  const handleDateChange: CalendarProps["onChange"] = (value) => {
    ...
    const dayActivities = actividades.filter(
      (actividad) => isSameDay(parseISO(actividad.startDate), selectedDate) // Ajustar según el formato de fecha recibido
    );
  ...
  const tileContent: CalendarProps["tileContent"] = ({ date }) => {
    const dayActivities = actividades.some(
      (actividad) => isSameDay(parseISO(actividad.startDate), date) // Ajustar según el formato de fecha recibido
    );
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold p-4">Calendario</h1>
            ...
            {showPanel && (
              <div className="activity-panel absolute top-0 left-0 right-0 mx-auto mt-6 p-4 bg-white rounded-lg shadow-lg w-20 md:w-96 z-20">
                ...
                <h2 className="text-sm md:text-lg font-bold mb-2">
                  Actividades para el {formatDate(date!)}
                </h2>
                <ul>
