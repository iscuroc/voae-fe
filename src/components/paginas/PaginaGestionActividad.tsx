...
const PaginaGestionActividad: React.FC = () => {
  ...
  const aplicarFiltros = (
    ...
    const fechaInicioDate = fechaInicio
      ? new Date(fechaInicio.split("T")[0])
      : null;
    ...
    const filtrar = filtrarData.filter((item) => {
      ...
      return (
        (carrera === "" ||
          item.foreingCareers.some((fc) => fc.name === carrera)) &&
        (ambito === "" ||
          item.scopes.some(
            (s) => EtiquetasAmbitosActividad[s.scope] === ambito
          )) &&
  ...
  const paginatedData = filtrarData.slice(
    (paginaInicial - 1) * itemsPerPage,
    paginaInicial * itemsPerPage
  ); // Usar FiltrarData en lugar de initialData
  ...
  if (error) {
    return (
      <div className="text-2xl font-bold text-center mt-20">Error: {error}</div>
    );
  }

  return (
    <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-6">Todas las Actividades</h1>
          ...
          <table className="border-collapse block md:table min-w-full table-auto bg-white border border-gray-200">
            ...
            <tbody className="block md:table-row-group text-sm md:text-xs">
              {paginatedData.map((item, index) => (
                <tr
                  key={index}
                  className="bg-yellow-500 md:bg-white text-left md:text-center hover:bg-gray-200 transition-colors duration-200 border border-gray-500 md:border-none block md:table-row"
                >
                  <td className="p-1 md:border md:border-gray-500 block md:table-cell">
                    <span className="inline-block w-1/3 md:hidden font-bold mr-4">
                      Nombre:
                    </span>
                    {item.name}
                  </td>
                  ...
                  <td className="p-1 md:border md:border-gray-500 block md:table-cell">
                    ...
                    {item.organizers
                      .map((fc) => fc.career?.name || fc.organization?.name)
                      .join(", ")}
                  </td>
                  <td className="p-1 md:border md:border-gray-500 block md:table-cell">
                    <span className="inline-block w-1/3 md:hidden font-bold mr-4">
                    ...
                    {item.requestedBy.names} {item.requestedBy.lastNames}
                  </td>
                  <td className="p-1 md:border md:border-gray-500 block md:table-cell">
                    <span className="inline-block w-1/3 md:hidden font-bold mr-4">
                      Ámbito:
                    ...
                    {item.scopes
                      .map((s) => EtiquetasAmbitosActividad[s.scope] || s.scope)
                      .join(", ")}
                  </td>
                  <td className="p-1 md:border md:border-gray-500 block md:table-cell">
                    <span className="inline-block w-1/3 md:hidden font-bold mr-4">
                  ...
                  <td className="p-1 md:border text-center md:border-gray-500 block md:table-cell relative">
                    ...
                    >
                      ...
                      <span className="hover:text-blue-500 group-hover:text-blue-500 md:hidden">
                        Revisar
                      </span>
                    </NavLink>
                  </td>
